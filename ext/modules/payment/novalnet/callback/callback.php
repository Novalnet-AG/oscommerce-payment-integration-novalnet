...
if (!empty($request_params['vendor_activation'])) {
    ...
    switch ($paymentlevel) {
        ...
        case 0: // Type of payment available
            if ($nncapture_params['subs_billing'] == 1 && (in_array($nncapture_params['tid_status'], array('100', '90', '91','98', '99', '86', '85')))) {
            ...
            } else if(in_array($nncapture_params['payment_type'],array('GUARANTEED_INVOICE','GUARANTEED_DIRECT_DEBIT_SEPA','INVOICE_START','DIRECT_DEBIT_SEPA')) && in_array($nncapture_params['tid_status'], array(91,99,100)) && $nncapture_params['status'] == 100 && in_array($nntrans_history['gateway_status'] ,array(75,91,99))){
                $param = array();
                ...
                if( in_array($nncapture_params['tid_status'],array(99,91)) && $nntrans_history['gateway_status'] == 75){
                    $order_status = ($nncapture_params['payment_type'] == 'GUARANTEED_INVOICE') ? ((constant('MODULE_PAYMENT_NOVALNET_INVOICE_CALLBACK_ORDER_STATUS') > 0) ? constant('MODULE_PAYMENT_NOVALNET_INVOICE_CALLBACK_ORDER_STATUS') : DEFAULT_ORDERS_STATUS_ID) : ((constant('MODULE_PAYMENT_'.strtoupper($nntrans_history['payment_type']).'_ORDER_STATUS') > 0) ? constant('MODULE_PAYMENT_'.strtoupper($nntrans_history['payment_type']).'_ORDER_STATUS') : DEFAULT_ORDERS_STATUS_ID);
                    $callback_comments .= PHP_EOL. sprintf(MODULE_PAYMENT_GUARANTEE_PAYMENT_PENDING_TO_HOLD_MESSAGE, $nncapture_params['shop_tid'],date(DATE_FORMAT, strtotime(date('d.m.Y'))), date('H:i:s'));
                } else if($nncapture_params['tid_status'] == 100 && in_array( $nntrans_history['gateway_status'], array(75,91,99))){
                    if((($nncapture_params['payment_type'] == 'GUARANTEED_INVOICE') &&  $nntrans_history['gateway_status'] == 75)){
                    ...
                    } elseif((($nncapture_params['payment_type'] == 'GUARANTEED_DIRECT_DEBIT_SEPA') &&  $nntrans_history['gateway_status'] == 75)) {
                        ...
                        $order_status = ((MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_COMPLETE > 0) ? MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_COMPLETE : DEFAULT_ORDERS_STATUS_ID);
                    }
                    ...
                    if(in_array($nncapture_params['payment_type'],array('GUARANTEED_INVOICE','INVOICE_START')) && in_array($nntrans_history['gateway_status'], array(75, 91))){

                        $serialize_data = unserialize($nntrans_history['payment_details']);
                        
                        list($transactionCommentsForm, $bank_details)  = NovalnetUtil::formInvoicePrepaymentComments(array(
                        ...
                        $transactionCommentsForm .= NovalnetUtil::novalnetReferenceComments($nntrans_history['payment_ref'],$nntrans_history['order_no'],$nntrans_history['payment_type'],$vendor_details);
...
class NovalnetVendorScript
{
    ...
    protected $chargebacks = array('RETURN_DEBIT_SEPA', 'CREDITCARD_BOOKBACK', 'CREDITCARD_CHARGEBACK', 'PAYPAL_BOOKBACK', 'REFUND_BY_BANK_TRANSFER_EU', 'PRZELEWY24_REFUND', 'REVERSAL','GUARANTEED_SEPA_BOOKBACK', 'GUARANTEED_INVOICE_BOOKBACK');
    ...
    protected $paymentgroups = array(
        ...
        'novalnet_sepa'       => array('DIRECT_DEBIT_SEPA', 'RETURN_DEBIT_SEPA', 'SUBSCRIPTION_STOP', 'DEBT_COLLECTION_SEPA', 'CREDIT_ENTRY_SEPA', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'REFUND_BY_BANK_TRANSFER_EU','GUARANTEED_SEPA_BOOKBACK'),
        ...
        'novalnet_invoice'    => array('INVOICE_START', 'INVOICE_CREDIT', 'SUBSCRIPTION_STOP', 'GUARANTEED_INVOICE', 'GUARANTEED_INVOICE_BOOKBACK'),
    ...
    function validateIpAddress()
    {
        ...
        $client_ip = (filter_var($client_ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) || empty($client_ip)) ? '127.0.0.1' : $client_ip;
    ...
    function validateCaptureParams($arycapture)
    {
        if (!isset($arycapture['vendor_activation'])) {
            ...
            if (!in_array($arycapture['payment_type'], array_merge($this->payments, $this->chargebacks, $this->debit_collections, $this->subscription)) && $arycapture['payment_type'] != 'TRANSACTION_CANCELLATION') {
    ...
    {
        $select_query = tep_db_query("SELECT order_no, amount, payment_id, payment_type,language,callback_amount,gateway_status,payment_details,payment_ref from novalnet_transaction_detail where tid = '" . tep_db_input($this->arycaptureparams['shop_tid']) . "'");
    ...
    function sendNotifyMail($datas = array(), $order_detail = true)
    {
        if (MODULE_PAYMENT_NOVALNET_CALLBACK_MAIL_SEND == 'True') {
            ...
            if (!empty($email_bcc)) {
                ...
                tep_mail(STORE_OWNER, $email_to, $email_subject, $email_content, STORE_NAME, $email_bcc);
            } else {
    ...
    function transactionCancellation($nntrans_history) {
		$nncapture_params = $this->getCaptureParams();
		if ($nncapture_params['payment_type'] == 'TRANSACTION_CANCELLATION' && in_array($nntrans_history['gateway_status'], array('75','91','99'))) {
            //Update callback order status due to full payment
            tep_db_perform(TABLE_ORDERS, array(
                'orders_status' => ((MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_CANCELLED > 0) ? MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_CANCELLED : DEFAULT_ORDERS_STATUS_ID)
            ), 'update', 'orders_id="' . $nntrans_history['order_no'] . '"');
            // To form the callback comments
            $callback_comments = PHP_EOL.sprintf('Novalnet callback received. The transaction has been canceled on %s %s',date(DATE_FORMAT, strtotime(date('d.m.Y'))), date('H:i:s')) . PHP_EOL;
           $param ['gateway_status'] = $nncapture_params['tid_status'];
           tep_db_perform('novalnet_transaction_detail', $param, "update", "tid='" . $nncapture_params['shop_tid'] . "'"); 
           ...
           'order_no'        => $nntrans_history['order_no'],
           'orders_status_id' => ((MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_CANCELLED > 0) ? MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_CANCELLED : DEFAULT_ORDERS_STATUS_ID),
            'comments'         => $callback_comments
            ));
            // Send notification mail to Merchant
