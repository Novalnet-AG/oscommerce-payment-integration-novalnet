...
class NovalnetWebhook
{
    ...
    public function __construct()
    {
        ...
        // Set event data.
        $GLOBALS['languages_id'] = tep_db_fetch_array(tep_db_query("SELECT language_id FROM " . TABLE_ORDERS . " WHERE orders_id={$this->event_data['transaction']['order_no']} limit 1"))['language_id'];
    ...
    public function get_order_reference()
    {
        ...
        if (empty($novalnet_order_details['order_no']) && !empty($this->event_data['transaction']['order_no'])) {
            ...
            $order_number = !empty($orders_id['orders_id']) ? $orders_id['orders_id'] : $$novalnet_order_details['order_no'];
    ...
    public function handle_transaction_capture()
    {
        ...
        $comments .= NovalnetHelper::get_comments($this->event_data);
        ...
        if ($this->event_data['transaction']['update_type'] == 'STATUS') {
            ...
            if (in_array($this->event_data['transaction']['status'], ['ON_HOLD', 'CONFIRMED'])) {
                ...
                if ($this->event_data['transaction']['status'] == 'CONFIRMED') {
                    ...
                    if (!empty($this->event_data['instalment'])) {
                        $novalnet_table_data['instalment_details'] = NovalnetHelper::store_instalment_details($this->event_data);
                    }
                }
    ...
    public function handle_instalment_cancel()
    {
        ...
        if (!empty($instalment_details)) {
            ...
            if ($this->event_data['instalment']['cancel_type'] == 'ALL_CYCLES') {
                $comments = PHP_EOL . sprintf(MODULE_PAYMENT_NOVALNET_PAYMENTS_INSTALMENT_CANCEL_ALLCYCLES_TEXT, $this->parent_tid, gmdate('d.m.Y'), $this->currency->format($amount, false, $this->event_data['transaction']['refund']['currency']));
            }
    ...
    public function send_webhook_mail($email_text)
    {
        ...
        $mail = !empty($mail) ? $mail : STORE_OWNER_EMAIL_ADDRESS;
        if (\common\helpers\Validations::validate_email($mail) == true) {
            ...
            $this->display_message(['message' => 'Vaild Email address is required']);
        }
    }
