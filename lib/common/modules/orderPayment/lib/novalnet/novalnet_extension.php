...
$get_order_details = NovalnetHelper::get_novalnet_transaction_details($order_id);
// Form the request parameter for CURL send to novalnet server
$data = [];
$data['transaction'] = [
    'tid' => $get_order_details['tid'],
];
if ($event == 'REFUND') {
    ...
    $data['transaction']['payment_type'] = $get_order_details['payment_type'];
    $data['transaction']['order_no'] = $order_id;
    $data['transaction']['amount'] = $amount;
    $data['transaction']['payment_data']['token'] = $get_order_details['payment_details'];
    $data['merchant'] = [
...
if ($response['result']['status'] == 'SUCCESS' && $event == 'CONFIRM') {
    ...
    if (in_array($response['transaction']['payment_type'], array('INVOICE', 'GUARANTEED_INVOICE'))) {
        $calc = $response['transaction']['amount'] / 100;
        $amount = $currencies->format($calc, false, $response['transaction']['currency']);
        $order_status_id = NovalnetHelper::get_order_status_id($response['transaction']);
...
} elseif ($response['result']['status'] == 'SUCCESS' && $event == 'CANCEL') {
    $novalnet_table_data = array(
        'status' => $response['transaction']['status'],
    );
    $order_status_id = NovalnetHelper::get_order_status_id($response['transaction']);
    $comments = MODULE_PAYMENT_NOVALNET_PAYMENTS_CALLBACK_EVENT_TRANSACTION_CANCEL_COMMENT . ' ' . date("d-m-Y");
} elseif ($response['result']['status'] == 'SUCCESS' && $event == 'REFUND') {
    $refund_amount = $response['transaction']['refund']['amount'] / 100;
    $refunded_amount = NovalnetHelper::get_novalnet_transaction_details($response['transaction']['order_no'])['refund_amount'] + $response['transaction']['refund']['amount'];
    ...
    if (NovalnetHelper::get_novalnet_transaction_details($response['transaction']['order_no'])['amount'] > $refunded_amount) {
        $order_status_id = 100027;
        $novalnet_table_data['status'] = 'CONFIRMED';
    }
} elseif ($response['result']['status'] == 'SUCCESS' && $event == 'BOOK_AMOUNT') {
