...
class NovalnetHelper
{
    ...
    {
        $total_amount = round($order_info['total'] * $order_info['currency_value'], 2) * 100;
        $data['transaction'] = [
            'amount' => $total_amount,
            'system_name' => PROJECT_VERSION_NAME,
            'system_version' => PROJECT_VERSION_MAJOR . '_' . PROJECT_VERSION_MINOR . '-NN13.0.0',
        ...
        if (in_array($response['transaction']['payment_type'], array('INVOICE', 'PREPAYMENT', 'GUARANTEED_INVOICE', 'INSTALMENT_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA'))) {
            ...
            if (!in_array($response['transaction']['payment_type'], array('INVOICE', 'PREPAYMENT'))) { // Except Invoice & Prepayment
                if ($response['transaction']['status'] === 'PENDING') { // Only for PENDING status
                    $bank_details_note = MODULE_PAYMENT_NOVALNET_PAYMENTS_GUARANTEE_PAYMENT_PENDING_TEXT . PHP_EOL;
                }
    ...
    public static function get_bank_account_details($response)
    {
        ...
        if ($response['transaction']['status'] !== 'ON_HOLD') { // If not equal to on hold
            $bank_details_note = !empty($response['instalment']) ? PHP_EOL . sprintf(MODULE_PAYMENT_NOVALNET_PAYMENTS_INSTALMENT_AMOUNT_TRANSFER_NOTE_DUE_DATE, $amount, $response['transaction']['due_date']) . PHP_EOL : PHP_EOL . sprintf(MODULE_PAYMENT_NOVALNET_PAYMENTS_AMOUNT_TRANSFER_NOTE_DUE_DATE, $amount, $response['transaction']['due_date']) . PHP_EOL;
        }
    ...
    public static function store_instalment_details($response)
    {
        ...
        $last_cycle_amount = $total_amount - ($cycle_amount * (isset($total_cycles) ? $total_cycles : null - 1));
    ...
    public static function instalment_details($response)
    {
        $comments = '';
        $currencies = \Yii::$container->get('currencies');
        $response['instalment']['cycle_amount'] = $response['instalment']['cycle_amount'] / 100;
        $amount = $currencies->format($response['instalment']['cycle_amount'], false, $response['instalment']['currency']);
    ...
    {
        $admin_mail = tep_db_fetch_array(tep_db_query("SELECT `admin_email_address` FROM  admin"));
    ...
    public static function get_prefill_amount($order_no)
    {
        ...
        return round($order_total['value'] * $order_total['currency_value'], 2) * 100;
    }
