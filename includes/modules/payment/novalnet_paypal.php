...
class novalnet_paypal
{
    ...
    function novalnet_paypal()
    {
		...
		$this->desc = (MODULE_PAYMENT_NOVALNET_PAYPAL_SHOP_TYPE == 'ONECLICK') ? MODULE_PAYMENT_NOVALNET_PAYPAL_ONE_CLICK_SHOPPING_DESCRIPTION : MODULE_PAYMENT_NOVALNET_PAYPAL_TEXT_DESC;
        ...
        $this->sort_order = defined('MODULE_PAYMENT_NOVALNET_PAYPAL_SORT_ORDER') ? MODULE_PAYMENT_NOVALNET_PAYPAL_SORT_ORDER : 0;
    ...
    function update_status()
    {
		...
		if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_NOVALNET_PAYPAL_PAYMENT_ZONE > 0) ) {
			$check_flag = false;
			$check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_NOVALNET_PAYPAL_PAYMENT_ZONE . "' and zone_country_id = '" . $order->delivery['country']['id'] . "' order by zone_id");
    ...
    function process_button()
    {
        ...
        if(($order_amount >= trim(MODULE_PAYMENT_NOVALNET_MANUAL_CHECK_PAYPAL_LIMIT) && MODULE_PAYMENT_NOVALNET_PAYPAL_AUTHENTICATE == 'authorize' ) || (empty (MODULE_PAYMENT_NOVALNET_MANUAL_CHECK_PAYPAL_LIMIT) && MODULE_PAYMENT_NOVALNET_PAYPAL_AUTHENTICATE == 'authorize' ) && (MODULE_PAYMENT_NOVALNET_PAYPAL_SHOP_TYPE != 'ZEROAMOUNT'  || $urlparam['amount'] != '0')) { // Assigning on hold parameter
                $urlparam['on_hold'] = 1;
            }	    
    ...
    function before_process()
    {
        ...
        if (MODULE_PAYMENT_NOVALNET_PAYPAL_SHOP_TYPE == 'ONECLICK') {
            ...
            $order_amount = $order->info['total']*100;
			if(($order_amount >= trim(MODULE_PAYMENT_NOVALNET_MANUAL_CHECK_PAYPAL_LIMIT) && MODULE_PAYMENT_NOVALNET_PAYPAL_AUTHENTICATE == 'authorize' ) || (empty (MODULE_PAYMENT_NOVALNET_MANUAL_CHECK_PAYPAL_LIMIT) && MODULE_PAYMENT_NOVALNET_PAYPAL_AUTHENTICATE == 'authorize' )){
                $urlparam['on_hold'] = 1;
            }	
		...
		if ($datas['status'] == 100) {
                    ...
                    $trans_comments                    = MODULE_PAYMENT_NOVALNET_TRANSACTION_DETAILS . PHP_EOL . MODULE_PAYMENT_NOVALNET_TRANSACTION_ID . $datas['tid'] . (((isset($datas['test_mode']) && $datas['test_mode'] == 1) || MODULE_PAYMENT_NOVALNET_PAYPAL_TEST_MODE == 'True') ? PHP_EOL . MODULE_PAYMENT_NOVALNET_TEST_ORDER_MSG . PHP_EOL : '');
                    $order->info['comments']           .= PHP_EOL.$trans_comments;
                } else {
    ...
    {
        global $insert_id;
        $payment_order_status          = $_SESSION['novalnet'][$this->code]['gateway_status'] =='90' ?MODULE_PAYMENT_NOVALNET_PAYPAL_PENDING_ORDER_STATUS : ($_SESSION['novalnet'][$this->code]['gateway_status'] =='85' ?MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_COMPLETE : MODULE_PAYMENT_NOVALNET_PAYPAL_ORDER_STATUS);
    ...
    function install()
    {
        ...
        ('" . MODULE_PAYMENT_NOVALNET_MANUAL_CHECK_PAYPAL_LIMIT_TITLE . "','MODULE_PAYMENT_NOVALNET_MANUAL_CHECK_PAYPAL_LIMIT', '', '6', '".MODULE_PAYMENT_NOVALNET_MANUAL_CHECK_PAYPAL_LIMIT_DESC."' , '7', '', '', now()),
        ('" . MODULE_PAYMENT_NOVALNET_PAYPAL_ORDER_STATUS_TITLE . "','MODULE_PAYMENT_NOVALNET_PAYPAL_ORDER_STATUS', '0', '6','" . MODULE_PAYMENT_NOVALNET_PAYPAL_ORDER_STATUS_DESC . "', '7', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now()),
                ('" . MODULE_PAYMENT_NOVALNET_PAYPAL_AUTHENTICATE_TITLE . "','MODULE_PAYMENT_NOVALNET_PAYPAL_AUTHENTICATE','capture', '6','" . MODULE_PAYMENT_NOVALNET_PAYPAL_AUTHENTICATE_DESC . "', '2', 'tep_mod_select_option(array(\'capture\' => MODULE_PAYMENT_NOVALNET_CAPTURE,\'authorize\' => MODULE_PAYMENT_NOVALNET_AUTHORIZE),\'MODULE_PAYMENT_NOVALNET_PAYPAL_AUTHENTICATE\'," . MODULE_PAYMENT_NOVALNET_PAYPAL_AUTHENTICATE . ",', '', now()),
    ...
    function validateAdminConfiguration($admin = false)
    {
        if (MODULE_PAYMENT_NOVALNET_PAYPAL_STATUS == 'True') {
            if (( MODULE_PAYMENT_NOVALNET_MANUAL_CHECK_PAYPAL_LIMIT !='' && !ctype_digit(MODULE_PAYMENT_NOVALNET_MANUAL_CHECK_PAYPAL_LIMIT) ) || (MODULE_PAYMENT_NOVALNET_PAYPAL_VISIBILITY_BY_AMOUNT != '' && !ctype_digit(MODULE_PAYMENT_NOVALNET_PAYPAL_VISIBILITY_BY_AMOUNT))) {
                if ($admin)
