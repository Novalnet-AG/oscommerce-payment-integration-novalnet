...
class novalnet_sofortbank
{
    ...
    function novalnet_sofortbank()
    {
        ...
        if (strpos(MODULE_PAYMENT_INSTALLED, $this->code) !== false) {
            $this->sort_order = MODULE_PAYMENT_NOVALNET_SOFORTBANK_SORT_ORDER;
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_NOVALNET_SOFORTBANK_PAYMENT_ZONE > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_NOVALNET_SOFORTBANK_PAYMENT_ZONE . "' and (zone_id < 1 or zone_id = " . $order->billing['zone_id'] . " ) and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection()
    {
        ...
        $selection['module'] .= '<br />' . $this->description . MODULE_PAYMENT_NOVALNET_REDIRECT_NOTICE_MSG . '<br />' . trim(strip_tags(MODULE_PAYMENT_NOVALNET_SOFORTBANK_CUSTOMER_INFO));
    ...
    {
        global $insert_id;
        if (MODULE_PAYMENT_NOVALNET_CONFIG_ENABLE_NOTIFICATION_FOR_TEST_TRANSACTION == 'True' && MODULE_PAYMENT_NOVALNET_SOFORTBANK_TEST_MODE != 'True') {
    ...
    function install()
    {
        ...
        ('" . MODULE_PAYMENT_NOVALNET_SOFORTBANK_TRANS_REFERENCE1_TITLE . "','MODULE_PAYMENT_NOVALNET_SOFORTBANK_TRANS_REFERENCE1', '', '6','" . MODULE_PAYMENT_NOVALNET_SOFORTBANK_TRANS_REFERENCE1_DESC . "', '8', '', '', now()),
        ('" . MODULE_PAYMENT_NOVALNET_SOFORTBANK_TRANS_REFERENCE2_TITLE . "','MODULE_PAYMENT_NOVALNET_SOFORTBANK_TRANS_REFERENCE2', '', '6', '" . MODULE_PAYMENT_NOVALNET_SOFORTBANK_TRANS_REFERENCE2_DESC . "','9', '', '', now())");
    }
    ...
    function keys()
    {
        ...
        return array(
            ...
            'MODULE_PAYMENT_NOVALNET_SOFORTBANK_TRANS_REFERENCE1',
            'MODULE_PAYMENT_NOVALNET_SOFORTBANK_TRANS_REFERENCE2'
        );
    }
