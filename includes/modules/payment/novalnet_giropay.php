...
class novalnet_giropay
{
    ...
    function novalnet_giropay()
    {
        ...
        $this->sort_order = defined('MODULE_PAYMENT_NOVALNET_GIROPAY_SORT_ORDER') ? MODULE_PAYMENT_NOVALNET_GIROPAY_SORT_ORDER : 0;
    ...
    function update_status()
    {
        ...
        if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_NOVALNET_GIROPAY_PAYMENT_ZONE > 0) ) {
			$check_flag = false;
			$check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_NOVALNET_GIROPAY_PAYMENT_ZONE . "' and zone_country_id = '" . $order->delivery['country']['id'] . "' order by zone_id");
    ...
    function selection()
    {
        ...
        $selection['module'] .= '<br />' . MODULE_PAYMENT_NOVALNET_GIROPAY_TEXT_DESC . '<br />' . trim(strip_tags(MODULE_PAYMENT_NOVALNET_GIROPAY_CUSTOMER_INFO));
