...
class novalnet_invoice
{
    ...
    function selection()
    {
        ...
        $selection['module'] .= '<br />' . $this->description . '<br />' . trim(strip_tags(MODULE_PAYMENT_NOVALNET_INVOICE_CUSTOMER_INFO));
        ...
        if (!isset($_SESSION['novalnet'][$this->code]['tid']) && isset($guarantee_payment) && $guarantee_payment == 'guarantee' && $this->guranteePaymentInvoiceVerifcation($order)) { // Display guarantee payment date of birth field
            $customer_bday = isset($customer_details['customers_dob']) ? date('d-m-Y', strtotime($customer_details['customers_dob'])) : '';
			$selection['fields'][] = array(
    ...
    function pre_confirmation_check()
    {
        ...
        } elseif (MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_CONFIGURATION == 'True' && MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_FORCE_GUARANTEE_PAYMENT != 'True' && !NovalnetUtil::validateAge($post['novalnet_invoicebirthdate'])) { // Validate age
    ...
    function after_process()
    {
        ...
        $order_status['orders_status']                    = $dbVal['orders_status_id'] = (MODULE_PAYMENT_NOVALNET_INVOICE_ORDER_STATUS > 0) ? MODULE_PAYMENT_NOVALNET_INVOICE_ORDER_STATUS : DEFAULT_ORDERS_STATUS_ID;
		
		if($_SESSION['novalnet'][$this->code]['payment_id'] == 41) {
			$order_status['orders_status']  = $dbVal['orders_status_id'] = (MODULE_PAYMENT_NOVALNET_INVOICE_CALLBACK_ORDER_STATUS > 0) ? MODULE_PAYMENT_NOVALNET_INVOICE_CALLBACK_ORDER_STATUS : DEFAULT_ORDERS_STATUS_ID;
		}
        tep_db_perform(TABLE_ORDERS, $order_status, "update", "orders_id='$insert_id'");
        $dbVal['comments'] = $_SESSION['novalnet'][$this->code]['comments'] . $reference_comments;
        tep_db_perform(TABLE_ORDERS_STATUS_HISTORY, $dbVal, "update", "orders_id='$insert_id'");
    ...
    function install()
    {
        ...
        ('" . MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT_TITLE . "','MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT', '', '6', '" . MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT_DESC . "','18', '', '', now()),
        ('" . MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_FORCE_GUARANTEE_PAYMENT_TITLE . "','MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_FORCE_GUARANTEE_PAYMENT','True', '6','" . MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_FORCE_GUARANTEE_PAYMENT_DESC . "','16', 'tep_mod_select_option(array(\'True\' => MODULE_PAYMENT_NOVALNET_TRUE,\'False\' => MODULE_PAYMENT_NOVALNET_FALSE),\'MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_FORCE_GUARANTEE_PAYMENT\'," . MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_FORCE_GUARANTEE_PAYMENT . ",' , '', now())
        ");
    ...
    function validateAdminConfiguration($admin = false)
    {
        if (MODULE_PAYMENT_NOVALNET_INVOICE_STATUS == 'True') {
            if (MODULE_PAYMENT_NOVALNET_INVOICE_VISIBILITY_BY_AMOUNT != '' && !ctype_digit(MODULE_PAYMENT_NOVALNET_INVOICE_VISIBILITY_BY_AMOUNT)) {
                ...
                return false;
            } elseif(MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_CONFIGURATION == 'True' && ((MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT != '' && (!ctype_digit(MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT) || MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT < 2000)) || (MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT != '' && (!ctype_digit(MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT) || MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT > 500000)) || (MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT != '' && MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT != '' && (MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT <= MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT)))){
				if(MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT != '' && (!ctype_digit(MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT) || MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT < 2000)) {
					if ($admin)
				...
				elseif(MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT != '' && (!ctype_digit(MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT) || MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT > 500000)) {
					if ($admin)
						echo NovalnetUtil::novalnetBackEndShowError(MODULE_PAYMENT_NOVALNET_INVOICE_BLOCK_TITLE, MODULE_PAYMENT_NOVALNET_INVOICE_GURANTEE_PAYMENT_MAX_AMOUNT_ERROR_MSG);
					return false;
				}
				elseif(MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT != '' && MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT != '' && (MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT <= MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT)) {
					if ($admin)
						echo NovalnetUtil::novalnetBackEndShowError(MODULE_PAYMENT_NOVALNET_INVOICE_BLOCK_TITLE, MODULE_PAYMENT_NOVALNET_INVOICE_GURANTEE_PAYMENT_MAX_AMOUNT_ERROR_MSG);
					return false;
				}
    ...
    function updateTransComments($payment_response, $input_params)
    {
        ...
        $_SESSION['novalnet'][$this->code] = array_merge($_SESSION['novalnet'][$this->code], array(
            ...
            'payment_id'            => (MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_CONFIGURATION == 'True' && $this->guranteePaymentInvoiceVerifcation($order)) ? 41: 27,
    ...
    {
        $minimum_amount = (MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT != '') ? MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MINIMUM_AMOUNT : 2000;
        $maximum_amount = (MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT != '') ? MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PAYMENT_MAXIMUM_AMOUNT : 500000;
        return (in_array($order->customer['country']['iso_code_2'], array('AT', 'DE', 'CH'
        )) && $order->info['currency'] == 'EUR' && NovalnetUtil::addressVerification($order) && isset($_SESSION['novalnet'][$this->code]['novalnet_invoicebirthdate']) && NovalnetUtil::validateAge($_SESSION['novalnet'][$this->code]['novalnet_invoicebirthdate']) && (NovalnetUtil::getPaymentAmount((array) $order) >= $minimum_amount && NovalnetUtil::getPaymentAmount((array) $order) <= $maximum_amount));
    }
}
?>
