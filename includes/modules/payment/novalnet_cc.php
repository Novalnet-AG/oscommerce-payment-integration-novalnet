...
class novalnet_cc
{
    ...
    function selection()
    {
        ...
        if (MODULE_PAYMENT_NOVALNET_CC_3D_SECURE == 'True' || (!isset($payment_details['cc_one_click']))) {
            ...
            $selection['module'] .= '<br />' . $end_customer_info . $test_mode_msg;
            $selection['fields'][] = array(
                'title' => '',
    ...
    function renderIframe()
    {
        ...
        $apiKey    = trim(MODULE_PAYMENT_NOVALNET_PRODUCT_ACTIVATION_KEY);
        $remote_ip = tep_get_ip_address();
        $remoteIp  = (filter_var($remote_ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) || $remote_ip == '::1' || empty($remote_ip)) ? '127.0.0.1' : $remote_ip;
        ...
        $signature = base64_encode($apiKey . '&' . $remoteIp . '&' . $serverIp);
