...
class novalnet_config
{
    ...
    {	
        ...
        if (!$insert_novalnet_tables) {
            ...
            $sql_version    = tep_db_query("select version from novalnet_version_detail where version='11.1.5'");
            $version_detail = tep_db_fetch_array($sql_version);
            if (empty($version_detail)) {
                tep_db_query("INSERT INTO novalnet_version_detail VALUES ('11.1.5')");
            }
        } else {
