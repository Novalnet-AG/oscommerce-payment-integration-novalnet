...
class NovalnetUtil
{
    ...
    static public function checkMerchantConfiguration($admin = false)
    {
        ...
        if (!function_exists('base64_decode') || !function_exists('base64_encode') || !function_exists('md5') || !function_exists('curl_init') || !function_exists('crc32') || !function_exists('pack')) {
            ...
            return false;
        } else if (empty(MODULE_PAYMENT_NOVALNET_VENDOR_ID)) {
                if ($admin)
                    echo self::novalnetBackEndShowError(MODULE_PAYMENT_NOVALNET_CONFIG_BLOCK_TITLE);
                    return false;
            }
            
        elseif (trim(MODULE_PAYMENT_NOVALNET_TARIFF_PERIOD) != '' && !preg_match('/^\d+(d|m|y){1}$/',MODULE_PAYMENT_NOVALNET_TARIFF_PERIOD) && trim(MODULE_PAYMENT_NOVALNET_VENDOR_ID) == '' ) {
        ...
        elseif (trim(MODULE_PAYMENT_NOVALNET_TARIFF_PERIOD2) != '' && trim(MODULE_PAYMENT_NOVALNET_TARIFF_PERIOD2_AMOUNT) == '' && trim(MODULE_PAYMENT_NOVALNET_VENDOR_ID) == '' ) {    if ($admin)
        ...
        elseif (trim(MODULE_PAYMENT_NOVALNET_TARIFF_PERIOD2_AMOUNT) != '' && !preg_match('/^\d+(d|m|y){1}$/',MODULE_PAYMENT_NOVALNET_TARIFF_PERIOD2) && trim(MODULE_PAYMENT_NOVALNET_VENDOR_ID) == ''  ) {
            if ($admin)
    ...
    static public function getRequestParams($datas)
    {
        ...
        $urlparam            = array(
            ...
            'system_ip'        => (filter_var($system_ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) || empty($system_ip)) ? '127.0.0.1' : $system_ip,
            'system_url'       => ((ENABLE_SSL == true) ? HTTPS_SERVER : HTTP_SERVER.DIR_WS_CATALOG),
            'system_version'   => (function_exists('tep_get_version') ? tep_get_version() : PROJECT_VERSION) . '-NN11.1.5'
        );
    ...
    static public function generateEncode($data = '',$uniqid)
    {
        ...
        try {
             ...
             $data = htmlentities(base64_encode(openssl_encrypt($data, "aes-256-cbc", $_SESSION['nn_access_key'], true, $uniqid)));
    ...
    {  
        ...
        try {
            ...
            $data = openssl_decrypt(base64_decode($data), "aes-256-cbc", $_SESSION['nn_access_key'], true, $uniqid); 
    ...
    static public function generatemd5Value($datas)
    {
        ...
        return hash('sha256', ($datas['auth_code'].$datas['product'].$datas['tariff'].$datas['amount'].$datas['test_mode'].$datas['uniqid'].strrev($_SESSION['nn_access_key'])));
    ...
    static public function formInvoicePrepaymentComments($response)
    {
        ...
        $trans_comments   = '';
        if(!in_array($response['tid_status'], array(75,91))){
    ...
    static public function sepaDuedate()
    {
        ...
        $sepa_due_date_limit     = (($sepa_due_date_limit_tmp != '' && ($sepa_due_date_limit_tmp >= 2  || $sepa_due_date_limit_tmp <= 14 )) ? $sepa_due_date_limit_tmp : 7);
